  ...
  public function getServiceData() {
    ...
    if (!empty($nodes)) {
      ...
      foreach ($nodes as $key => $value) {
        ...
        $field_tags[] = $value->field_tags->target_id;
        if (in_array($author_name, $author_array) && in_array($author_name, $author_array)) {
          $text = $value->title->value . " Term Id =  ". $value->field_tags->target_id." By author is : ".$author_name;
          $result = Link::fromTextAndUrl(t($text), Url::fromUri('internal:/node/'.$value->nid->value, $options))->toString();
          $output[$value->nid->value] .= $result;
        }
      }
    }
